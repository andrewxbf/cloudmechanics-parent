...

{
    ...
    public static String getCopy(String source, int times) {
        if (StringUtil.isNullString(source)) {
            return StringUtil.getSpaces(times); // 如果是NULL或"",则返回times个空
        }
    ...
    static public String padLeftChar(String s, int len, String str) {
        ...
        String ret;
        if (slen < len) {
            ret = StringUtil.getCopy(str, len - slen) + s;
        } else {
    ...
    static public String padRightChar(String s, int len, String str) {
        ...
        String ret;
        if (slen < len) {
            ret = s + StringUtil.getCopy(str, len - slen);
        } else {
            ret = substring(s, len); // s.substring(0, len);
        }
        return ret;
    ...
    private static String replaceStr_old(String source, String oldString, String newString) {// deleted
                                                                                             // by
                                                                                             // lhy
                                                                                             // 201105262223
     ...
     * @return 替换后的字符串
     */
    public static String replaceStr(String source, String oldString, String newString) { // edited
                                                                                         // by
                                                                                         // lhy
                                                                                         // 201105262223
                                                                                         // 对当没有找到源串时候替换效率进行了提高,速度提高5倍
        if (oldString == null || oldString.length() == 0) {
            return source;
        }
        if (source == null) {
    ...
    public static String deleteRNT(String clause) {
        String result = StringUtil.replaceStr(clause, "\r", "");
        result = StringUtil.replaceStr(result, "\n", "");
        result = StringUtil.replaceStr(result, "\t", "");
        return result;
    }
